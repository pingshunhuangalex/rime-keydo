...
local function has_cn_char(text)
    for pos, code in utf8.codes(text) do
        ...
        if (byte1 >= 228 and byte1 <= 233) and (byte2 >= 128 and byte2 <= 191) and (byte3 >= 128 and byte3 <= 191) then
            return true
        end
...
local function hint(cand, context, reverse)
    ...
    local short = lookup:match(" (".. phonetics_keycode .. stroke_keycodes .. ") ") or
                  lookup:match(" (".. phonetics_keycode .. phonetics_keycode .. ") ")
    local input = context.input

    if short and #input > #short and not starts_with(short, input) then
...
local function sphs_filter(cand_list, env)
    ...
    for cand in cand_list:iter() do
        if cand.text == nil or cand.text == "" or (is_repeat_on and not has_cn_char(cand.text)) then
            goto continue
        end

        if is_sbb_hint_on and utf8.len(cand.text) > 1 then
            hint(cand, context, reverse)
        end
